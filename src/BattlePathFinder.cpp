...
void Battle::PathFinder::calculatePaths()
{
	...
	while (!this->pq.empty())
	{
		...
		for (PathNode* neighbour : neighbourNodes)
		{
			...
			if ((destination->coord == source->coord - sf::Vector2i(bigCreature, 0) || neighbour->accessible == PathNode::Accessibility::ACCESSIBLE || neighbour->accessible == PathNode::Accessibility::BLOCKVIS) && (pos2 == source->coord || neighbour2->accessible == PathNode::Accessibility::ACCESSIBLE || neighbour->accessible == PathNode::Accessibility::BLOCKVIS))
				push(neighbour);
		}
	}
}
